...
def get_link_names(wiki_rootdir, curr_filename):
    ...
    Note that the var $filename should have no extension, though the
    corresponding actual file name must have a .md extension
    ...
    '''
    TODO: Consider making this open file line not throw exception when file
    doesn't exist There just might be a link that doesn't yet point to a file.
    '''
    ...
    ref_filename = []
    for line in lines:
        # Find matches for links of the form: [reference text
        # here](reference_filename_here)
    ...
    for pair in ref_filename:
        ...
        # We don't really need this, but it may be useful for later
        # functionality
        refs.append(templist[0])
...
def dotlang_to_outfile(dotlang_filename):
    # NOTE: lower priority
    """
    ...
    """
    subprocess.run(['dot', '-Tsvg', '-O', dotlang_filename])
    # TODO: call browser to open dotlang_filename.svg
    # TODO: add option to choose file extension
